...
try:
    ...
    exit('ERROR: Cannot import the BeautifulSoup 3 Python library. Are you sure you installed it? (apt-get install '
...
def download_url(url, data=None, cookie=None, loop_client=0, loop_server=0):
    ...
    try:
        ...
        if random_proxy:
            proxyh = urllib.request.ProxyHandler({'http': random_proxy})
            opener = urllib.request.build_opener(proxyh, NoRedirectHandler())
        ...
        htmlPage = str(response.read(), errors='replace')
